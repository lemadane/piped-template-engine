...
public final class JavaCodeGenerator {
    ...
    public String generateClassSource(ASTNode rootNode, String className) {
        ...
        sb.append("package io.succinct.piped.template.engine.codegen.generated;\n\n");
        sb.append("import io.succinct.piped.template.engine.TemplateEngine;\n");
        sb.append("import io.succinct.piped.template.engine.codegen.CompiledTemplateExecutable;\n");
        sb.append("import io.succinct.piped.template.engine.expression.TemplateContext;\n");
    ...
    private void generateNodeSource(ASTNode node, StringBuilder sb, String indent) {
        if (node instanceof TextNode textNode) {
        ...
        } else if (node instanceof io.succinct.piped.template.engine.ast.FieldNode fieldNode) {
            sb.append(indent).append("new io.succinct.piped.template.engine.ast.FieldNode(")
                    .append(escapeStringLiteral(fieldNode.getPropertyPath())).append(", new io.succinct.piped.template.engine.expression.ExpressionEvaluator()).render(context, writer);\n");
        } else if (node instanceof io.succinct.piped.template.engine.ast.DisplayNode displayNode) {
            sb.append(indent).append("new io.succinct.piped.template.engine.ast.DisplayNode(")
                    .append(escapeStringLiteral(displayNode.getPropertyPath())).append(", new io.succinct.piped.template.engine.expression.ExpressionEvaluator()).render(context, writer);\n");
        } else if (node instanceof io.succinct.piped.template.engine.ast.EditorNode editorNode) {
            sb.append(indent).append("new io.succinct.piped.template.engine.ast.EditorNode(")
                    .append(escapeStringLiteral(editorNode.getPropertyPath())).append(", new io.succinct.piped.template.engine.expression.ExpressionEvaluator()).render(context, writer);\n");
        } else if (node instanceof io.succinct.piped.template.engine.ast.MacroNode macroNode) {
            sb.append(indent).append("new io.succinct.piped.template.engine.ast.MacroNode(")
        ...
        } else if (node instanceof io.succinct.piped.template.engine.ast.CallMacroNode callNode) {
            sb.append(indent).append("new io.succinct.piped.template.engine.ast.CallMacroNode(")
            ...
            sb.append("), new io.succinct.piped.template.engine.expression.ExpressionEvaluator()).render(context, writer);\n");
        } else if (node instanceof io.succinct.piped.template.engine.ast.SeparatorNode sepNode) {
            generateNodeSource(sepNode.getBody(), sb, indent);
        } else if (node instanceof io.succinct.piped.template.engine.ast.FragmentNode fragNode) {
            generateNodeSource(fragNode.getBody(), sb, indent);
        } else if (node instanceof io.succinct.piped.template.engine.ast.MinifyNode minifyNode) {
